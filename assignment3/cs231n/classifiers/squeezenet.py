...
class SqueezeNet(object):
    def extract_features(self, input=None, reuse=True):
        ...
        with tf.variable_scope('features', reuse=reuse):
            ...
            with tf.variable_scope('layer2'):
                x = tf.nn.max_pool(
                    x, [1, 3, 3, 1], strides=[1, 2, 2, 1], padding='VALID')
                layers.append(x)
    ...
    def __init__(self, save_path=None, sess=None):
        ...
        self.image = tf.placeholder(
            'float', shape=[None, None, None, 3], name='input_image')
        ...
        with tf.variable_scope('classifier'):
            ...
            with tf.variable_scope('layer3'):
                x = tf.nn.avg_pool(
                    x, [1, 13, 13, 1], strides=[1, 13, 13, 1], padding='VALID')
                self.layers.append(x)
        ...
        self.loss = tf.reduce_mean(
            tf.nn.softmax_cross_entropy_with_logits_v2(
                labels=tf.one_hot(self.labels, NUM_CLASSES),
