...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    ##############################################################################
    ht = prev_h.dot(Wh) + x.dot(Wx) + b  # (N, H)
    next_h = np.tanh(ht)
    cache = x, Wx, prev_h, Wh, ht
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return next_h, cache
...
def rnn_step_backward(dnext_h, cache):
    ...
    ##############################################################################
    x, Wx, prev_h, Wh, ht = cache
    dht = (1 - np.tanh(ht)**2) * dnext_h  # (N, H)
    dx = dht.dot(Wx.T)
    dWx = x.T.dot(dht)  # (D, H) = (D, N).(N, H)
    dWh = prev_h.T.dot(dht)  # (H, H) = (H, N).(N, H)
    db = np.sum(dht, axis=0)  # (H, )
    dprev_h = dht.dot(Wh.T)  # (N, H) = (N, H).(H, H)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    #############################################################################
    H = prev_h.shape[1]
    # activation function (N, D).(D, 4H) + (N, H).(H, 4H)
    ...
    ag = a[:, 3 * H:4 * H]
    gate_i = sigmoid(ai)
    gate_f = sigmoid(af)  # (N, H)
    gate_o = sigmoid(ao)
    gate_g = np.tanh(ag)
    next_c = gate_f * prev_c + gate_i * gate_g
    next_h = gate_o * np.tanh(next_c)
    cache = gate_i, gate_f, gate_o, gate_g, a, ai, af, ao, ag, Wx, Wh, b, prev_h, prev_c, x, next_c, next_h
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
