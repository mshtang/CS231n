...
def affine_forward(x, w, b):
    ...
    ###########################################################################
    N = x.shape[0]
    x_ = np.reshape(x, (N, -1))  # (N, D)
    out = x_.dot(w) + b  # (N, D) * (D, M) + (M,)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, w, b)
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        # variables.                                                          #
        #######################################################################
        # mini_inx = np.random.choice(N, int(N * 0.25), replace=False)
        # mini_batch = x[mini_inx]
        mini_batch_mean = np.mean(x, axis=0)
        mini_batch_variance = np.var(x, axis=0)
        # normalize x
        x_normalized = (x - mini_batch_mean) / \
            np.sqrt(mini_batch_variance + eps)
        out = gamma * x_normalized + beta
        running_mean = momentum * running_mean + \
            (1 - momentum) * mini_batch_mean
        running_var = momentum * running_var + \
            (1 - momentum) * mini_batch_variance

        cache = {'x_minus_mean': x - mini_batch_mean,
                 'normalized_data': x_normalized,
                 'gamma': gamma,
                 'ivar': 1. / np.sqrt(mini_batch_variance + eps),
                 'sqrtvar': np.sqrt(mini_batch_variance + eps)}
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
    elif mode == 'test':
...
def batchnorm_backward_alt(dout, cache):
    ...
    gamma = cache.get('gamma')
    ivar = cache.get('ivar')
    x_minus_mean = cache.get('x_minus_meam')
    sqrtvar = cache.get('sqrtvar')

    dbeta = np.sum(dout, axis=0)
    dgamma = np.sum(dout * normalized_data, axis=0)
    dx = 1/N*gamma*(1/sqrtvar)*((N*dout)-np.sum(dout, axis=0) -
                                (x_minus_mean)*np.square(ivar)*np.sum(dout*(x_minus_mean), axis=0))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    out_ = out.reshape(x_t.shape)
    out = out_.transpose((0, 3, 1, 2))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def spatial_batchnorm_backward(dout, cache):
    ...
    dx_ = dx.reshape(dout_t.shape)
    dx = dx_.transpose((0, 3, 1, 2))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx, dgamma, dbeta
